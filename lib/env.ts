...
export function getDatabaseUrl(): string {
  ...
  throw new Error("DATABASE_URL is not set. Copy `.env.example` to `.env.local` and configure PostgreSQL (see README).");
}
...
export function getSessionCookieSecure(): boolean {
  if (process.env.SESSION_COOKIE_SECURE !== undefined) return process.env.SESSION_COOKIE_SECURE !== "false";
  return isProduction();
}
