...
export default function Roster({
  ...
  const lastSubmission = participants.length
    ? dateTimeFmt.format(new Date(participants[participants.length - 1].submittedAt))
    : "—";
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="card-lux flex flex-wrap items-center gap-3 rounded-2xl p-4">
        ...
        <button onClick={() => setAddOpen(true)} className="btn-gold inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5 focus-within:border-gold-500/40">
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-sm font-bold text-ivory outline-none placeholder:text-ivory/30"
            />
            {search && (
              <button onClick={() => setSearch("")} className="text-[11px] font-bold text-ivory/40 hover:text-gold-300">
                مسح
              </button>
            )}
          </div>
          <p className="text-xs font-bold text-ivory/45">
            {search ? `نتائج البحث: ${filtered.length} من ${participants.length}` : `إجمالي الكشف: ${participants.length}`}
          </p>
        </div>
      </section>
      ...
      {participants.length === 0 ? (
        <div className="card-lux rounded-3xl p-14 text-center">
          ...
          <p className="font-display text-2xl font-bold text-ivory/70">لا توجد تقديمات بعد</p>
          <p className="mt-2 text-xs text-ivory/40">أول ما حد يسجل من الصفحة الرئيسية هيظهر هنا بصورته ووقت تقديمه</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="card-lux rounded-3xl p-12 text-center">
          <Search className="mx-auto mb-4 size-9 text-gold-500/40" />
          <p className="font-display text-xl font-bold text-ivory/70">مفيش اسم مطابق للبحث</p>
          <button onClick={() => setSearch("")} className="btn-ghost mt-4 rounded-xl px-5 py-2.5 text-xs font-bold">مسح البحث</button>
        </div>
      ) : (
            ...
            return (
              <article
                ...
                className={`overflow-hidden rounded-2xl border transition-all ${
                  isOpen
                    ? "border-gold-400/40 bg-gold-500/[0.04]"
                    : "border-white/10 bg-white/[0.025] hover:border-gold-500/30"
                } ${p.skipped ? "opacity-60" : ""}`}
              >
                <button
                  type="button"
                ...
                >
                  <span className="flex size-9 shrink-0 items-center justify-center rounded-full border border-gold-500/30 bg-gold-500/10 text-[11px] font-black text-gold-300">
                    #{actualIndex + 1}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="flex min-w-0 items-center gap-2">
                      <span className="truncate font-display text-lg font-bold text-ivory">{p.fullName}</span>
                      {hasDraft(p.id) && <span className="rounded-full border border-amber-400/40 bg-amber-500/15 px-2 py-0.5 text-[9px] font-black text-amber-300">مسودة</span>}
                    </span>
                    <span className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-ivory/40">
                      <span>{timeFmt.format(new Date(p.submittedAt))}</span>
                      <span>•</span>
                      <span>{p.skipped ? "متخطَّى من العرض" : "داخل العرض"}</span>
                      <span className={`rounded-full border px-2 py-0.5 ${chip.cls}`}>{chip.label}</span>
                    </span>
                  </span>
                ...
                <div className={`${isOpen ? "block" : "hidden"} border-t border-white/10 px-4 pb-4 pt-4`}>
                      ...
                      {editingId === p.id ? (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onKeyDown={(e) => { if (e.key === "Enter") { const v = editingName.trim(); if (v && v !== p.fullName) draftChange(p.id, "fullName", v, false); setEditingId(null); } if (e.key === "Escape") setEditingId(null); }}
                            className="input-lux w-full rounded-xl px-3 py-2.5 text-sm font-bold"
                          />
                          <button onClick={() => { const v = editingName.trim(); if (v && v !== p.fullName) draftChange(p.id, "fullName", v, false); setEditingId(null); }} className="rounded-xl bg-gold-500/20 p-2.5 text-gold-300"><Check className="size-4" /></button>
                        </div>
                      ) : (
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="font-display text-2xl font-bold text-ivory">{p.fullName}</h3>
                          <button onClick={() => { setEditingId(p.id); setEditingName(p.fullName); }} className="text-ivory/35 hover:text-gold-300" aria-label="تعديل الاسم"><Pencil className="size-4" /></button>
                        </div>
                      )}
                  ...
                  {confirmDelete === p.id && (
                    <div className="mt-4 rounded-2xl border border-red-400/30 bg-red-500/10 p-4">
                      <p className="text-sm font-black text-red-200">حذف {p.fullName} نهائيًا؟</p>
                      <p className="mt-1 text-[11px] text-ivory/50">سيتم حذف سجله وكل صوره (الأصلية والمولّدة)</p>
                      <div className="mt-3 flex gap-2">
                        <button onClick={() => doDelete(p)} className="btn-danger rounded-xl px-5 py-2 text-xs font-black">تأكيد الحذف</button>
                        <button onClick={() => setConfirmDelete(null)} className="btn-ghost rounded-xl px-5 py-2 text-xs font-black">تراجع</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <Modal
        ...
        title={replaceTarget?.kind === "adult" ? `تغيير الصورة الحالية — ${replaceTarget?.p.fullName ?? ""}` : `تغيير صورة الطفولة — ${replaceTarget?.p.fullName ?? ""}`}
      >
        {replaceTarget && (
          <div className="space-y-4">
            <UploadField
              ...
              label={replaceTarget.kind === "adult" ? "الصورة الحالية الجديدة" : "صورة الطفولة الجديدة"}
              oldStyle={replaceTarget.kind === "childhood"}
              value={replaceValue}
              onChange={setReplaceValue}
            />
            <p className="rounded-xl border border-amber-400/25 bg-amber-500/5 px-4 py-2.5 text-[11px] font-bold text-amber-200">الصورة الجديدة مسجلة كمسودة — هتتطبق وتحذف القديمة بعد الضغط على زرار «حفظ التعديلات»</p>
            <div className="flex gap-2.5">
              <button onClick={commitReplace} disabled={!replaceValue} className="btn-gold flex-1 rounded-xl py-3 text-sm">تسجيل التغيير كمسودة</button>
              <button onClick={() => setReplaceTarget(null)} className="btn-ghost rounded-xl px-6 py-3 text-sm font-bold">إلغاء</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="إضافة خريج من غرفة الإدارة" wide>
        ...
        <div className="mt-5 flex gap-2.5">
          <button onClick={submitAdd} disabled={!memberIsValid(addMember) || addBusy} className="btn-gold flex flex-1 items-center justify-center gap-2 rounded-xl py-3 text-sm">
            {addBusy ? <Loader2 className="size-4 animate-spin" /> : <Plus className="size-4" />} إضافة فورية
          </button>
          <button onClick={() => setAddOpen(false)} className="btn-ghost rounded-xl px-6 py-3 text-sm font-bold">إلغاء</button>
        </div>
      </Modal>
    </div>
  );
...
function IconBtn({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={`inline-flex items-center justify-center gap-1 rounded-xl border px-2.5 py-2 transition-all disabled:opacity-25 ${
        danger
...
function Stat({ label, value, small }: { label: string; value: string; small?: boolean }) {
  return (
    <div className="card-lux rounded-2xl px-5 py-4">
      ...
      <p className={`mt-1 font-display font-bold text-gold-200 ${small ? "text-sm" : "text-2xl"}`}>{value}</p>
    </div>
  );
}
